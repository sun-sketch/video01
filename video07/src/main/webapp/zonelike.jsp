...
<div class="order-by">
    <span>排序方式：</span>
    <a href="#" class="btn btn-primary" role="button" id="like">最多点赞</a>
    <a href="${pageContext.request.contextPath}/user/zonetime?zonename=<%=request.getParameter("zonename")%>&currentPage=1" class="btn btn-default" role="button" id="time">最近发布</a>
</div>
    ...
    <c:if test="${totalpage>1}">
        <nav aria-label="Page navigation" id="page">
            <ul class="pagination" id="page_ul">
                ...
                <c:if test="${currentPage != 1}">
                    <li>
                        <a href="${pageContext.request.contextPath}/user/zonelike?zonename=<%=request.getParameter("zonename")%>&currentPage=${currentPage - 1}" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                </c:if>
                <c:forEach begin="1" end="${totalpage}" var="i" >
                    <c:if test="${currentPage == i}">
                        <li class="active"><a class="pa" href="${pageContext.request.contextPath}/user/zonelike?zonename=<%=request.getParameter("zonename")%>&currentPage=${i}">${i}</a></li>
                    </c:if>
                    <c:if test="${currentPage != i}">
                        <li><a href="${pageContext.request.contextPath}/user/zonelike?zonename=<%=request.getParameter("zonename")%>&currentPage=${i}">${i}</a></li>
                    </c:if>
                </c:forEach>
                ...
                <c:if test="${currentPage != p.totalPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/user/zonelike?zonename=<%=request.getParameter("zonename")%>&currentPage=${currentPage + 1}" aria-label="Next">
                            <span aria-hidden="true">&raquo;</span>
                        </a>
                    </li>
                </c:if>
