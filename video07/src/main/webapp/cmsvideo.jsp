...
<head>
    ...
    <script>
        function paclick(i) {
            $.ajax({
                url:"${pageContext.request.contextPath}/admin/cmsvideo?currentPage="+i,
                type:"get",
                dataType:"html",
        ...
        function add() {
            $.ajax({
                url:"${pageContext.request.contextPath}/admin/cmsaddvideo?currentPage=1",
                type:"get",
                dataType:"html",
                success:function(msg){
                    $("#list").html(msg);
...
<div id="list">
    <div class="add">
        <button type="button" class="btn btn-primary add-btn" onclick="add()">审核视频</button>
    </div>
    <table class="table" id="mytable">
        ...
        <c:forEach items="${p.list}" var="video" varStatus="s">
            <tr>
                <td>${s.count+requestScope.p.begin}</td>
